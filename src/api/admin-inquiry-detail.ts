...
/** Fetch internal notes for an inquiry (from inquiry_internal_notes table if exists) */
export async function fetchInquiryInternalNotes(
  ...
  try {
    ...
    return list.map((row: { id: string; inquiry_id: string; author_id?: string; text: string; created_at: string; author?: { full_name?: string } }) => ({
      id: row.id,
      inquiryId: row.inquiry_id,
      authorName: (row as { author_name?: string }).author_name ?? 'Staff',
      text: row.text ?? '',
...
export async function createInquiryInternalNote(
  ...
  try {
    ...
    // Fallback: append to inquiry internal_notes string
...
export async function createStripePaymentLink(
  inquiryId: string,
  payload: CreateStripeLinkPayload
): Promise<{ paymentLinkUrl: string; paymentId: string }> {
  const url = `${SUPABASE_URL}/functions/v1/create-stripe-payment-link`
